...
void* sys_sbrk(int increment) {
    ...
    if (new_break > (uint32)env->da_limit) {
    ...
    if (increment > 0) {
        ...
        while (current_break <= new_break) {
           ...
           // map_frame(ptr_page_directory, frames, current_break, PERM_WRITEABLE);
        	allocate_user_mem(curenv,current_break,PAGE_SIZE);
...
uint32 sys_get_hard_limit_user()
{
	 return (curenv->da_limit);
}
/**************************************************************************/
...
uint32 syscall(uint32 syscallno, uint32 a1, uint32 a2, uint32 a3, uint32 a4, uint32 a5)
{
	...
	case SYS_GET_LIMIT:
		    return (uint32)sys_get_hard_limit_user();
		    break;
	case SYS_SBRK:
